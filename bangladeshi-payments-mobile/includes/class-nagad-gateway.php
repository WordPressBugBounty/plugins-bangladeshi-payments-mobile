...
?>
    ...
    public function process_payment($order_id) {
        ...
        if (
    isset( $_POST['nagad_payment_nonce'] )
    && wp_verify_nonce(
        sanitize_text_field( wp_unslash( $_POST['nagad_payment_nonce'] ) ),
        'nagad_payment_action'
    )
) {
    $phone = isset( $_POST['nagad_phone'] )
        ? sanitize_text_field( wp_unslash( $_POST['nagad_phone'] ) )
        : '';

    $trx   = isset( $_POST['nagad_transaction_id'] )
        ? sanitize_text_field( wp_unslash( $_POST['nagad_transaction_id'] ) )
        : '';
} else {
    wp_die(
        esc_html__( 'Security check failed. Please try again.', 'bangladeshi-payments-mobile' )
    );
}
