...
printf(
    ...
    wp_kses_post(
        sprintf(
            // translators: 1: Total payment amount. 2: UPay fees amount.
...
?>
    ...
    public function validate_fields() {
        $phone = isset($_POST['upay_phone']) ? sanitize_text_field(wp_unslash($_POST['upay_phone'])) : '';
        $trx   = isset($_POST['upay_transaction_id']) ? sanitize_text_field(wp_unslash($_POST['upay_transaction_id'])) : '';
    ...
    public function process_payment($order_id) {
        ...
        if (
            isset( $_POST['upay_payment_nonce'] )
            && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['upay_payment_nonce'] ) ), 'upay_payment_action' )
        ) {
            $phone = isset( $_POST['upay_phone'] ) ? sanitize_text_field( wp_unslash( $_POST['upay_phone'] ) ) : '';
            $trx   = isset( $_POST['upay_transaction_id'] ) ? sanitize_text_field( wp_unslash( $_POST['upay_transaction_id'] ) ) : '';
            ...
            wp_die( esc_html__( 'Security check failed. Please try again.', 'bangladeshi-payments-mobile' ) );
        }


        if ($phone) update_post_meta($order_id, '_upay_phone', $phone);
        if ($trx)   update_post_meta($order_id, '_upay_transaction_id', $trx);
    ...
    public function display_admin_order_info($order) {
        ...
        <div class="payment-order-page">
            <table>
                <tr><td colspan="2"><h4><?php esc_html_e('Upay Payment Information', 'bangladeshi-payments-mobile'); ?></h4></td></tr>
                <tr><td><?php esc_html_e('Payment Method:', 'bangladeshi-payments-mobile'); ?></td><td><?php esc_html_e('Upay', 'bangladeshi-payments-mobile'); ?></td></tr>
